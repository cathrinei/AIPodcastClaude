...
- `archive_episodes.py` — flytter episoder eldre enn 2 måneder fra hoved-CSV til `AI_KI_Podcasts_arkiv.csv`; kjøres daglig av GitHub Actions etter `auto_rate.py`; cutoff = rullerende daglig (`date.today() - relativedelta(months=2)` fra `python-dateutil`)
...
4. `archive_episodes.py` kjøres automatisk — episoder eldre enn 2 måneder flyttes fra `AI_KI_Podcasts.csv` til `AI_KI_Podcasts_arkiv.csv`
