...
- **`.ep-share-btn`** — 🔗-ikon per episode; i tabellen inline etter episodelenken, i mobilkort gruppert med `fav-btn` i en `flex`-span med `margin-left:auto`; klikk kopierer `buildEpisodeUrl()`-URL til utklippstavlen og viser ✓ i 1,5 sek; event delegation på `document`
